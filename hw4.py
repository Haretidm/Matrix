...
indep_vec_1 = Vec({0,1,2}, {0:one})
indep_vec_2 = Vec({0,1,2}, {1:one})
indep_vec_3 = Vec({0,1,2}, {2:one})
...
'''
    ...
    augL = S.copy()
    for testVec in L:
        if testVec not in augL:
            testL = augL.copy()
            testL.append(testVec)
            if not is_superfluous(testL, len(testL)-1):
                augL.append(testVec)
...
'''
    augL = S.copy()
    augL.append(z)
    for i in range(len(augL)):
        if augL[i] not in A and is_superfluous(augL, i):
            return S[i]