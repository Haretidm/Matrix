...
'''
    ...
    returnList = []
    for injectVec in B:
        ejectVec = hw4.exchange(C , B , injectVec)
        C.append(injectVec)
        C.remove(ejectVec)
        returnList.append( (injectVec , ejectVec) )
...
col_space_1 = [list2vec(v) for v in [[1,0],[2,2]]]
...
'''
    returnList = []
    for testVec in T:
        testList = returnList.copy()
        testList.append(testVec)
        if my_is_independent(testList):
            returnList.append(testVec)
...
True
'''
    rowList = U_basis + V_basis
    combVec = hw4.vec2rep(rowList , w)
    u = Vec(w.D , {})
    v = Vec(w.D , {})
    for i in range(len(rowList)):
        basisVec = rowList[i]
        if basisVec in U_basis:
            u += combVec[i]*basisVec
        else:
            pass
            v += combVec[i]*basisVec
...
'''
    inverseColDict = dict()
    identGF2Mat = Mat(A.D, {(d,d):one for d in A.D[0]})
    colList = mat2coldict(identGF2Mat)
    for k,standBasis in colList.items():
        inverseColDict[k] = solve(A,standBasis)
